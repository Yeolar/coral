...
inline std::string exceptionStr(const std::exception& e) {
  ...
  return folly::to<std::string>(demangle(typeid(e)), ": ", e.what());
#else
  return folly::to<std::string>("Exception (no RTTI available): ", e.what());
#endif
}
...
{
#ifdef FOLLY_HAS_RTTI
  return folly::to<std::string>(demangle(typeid(e)));
#else
