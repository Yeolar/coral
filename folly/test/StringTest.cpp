...
namespace {
string cbmString;
string cbmEscapedString;
string cEscapedString;
string cUnescapedString;
const size_t kCBmStringLength = 64 << 10;
const uint32_t kCPrintablePercentage = 90;

string uribmString;
string uribmEscapedString;
string uriEscapedString;
string uriUnescapedString;
...
void initBenchmark() {
  ...
  cbmEscapedString = cEscape<string>(cbmString);
  ...
  uribmEscapedString = uriEscape<string>(uribmString);
}

BENCHMARK(BM_cEscape, iters) {
  while (iters--) {
    cEscapedString = cEscape<string>(cbmString);
...
BENCHMARK(BM_cUnescape, iters) {
  while (iters--) {
    cUnescapedString = cUnescape<string>(cbmEscapedString);
...
BENCHMARK(BM_uriEscape, iters) {
  while (iters--) {
    uriEscapedString = uriEscape<string>(uribmString);
...
BENCHMARK(BM_uriUnescape, iters) {
  while (iters--) {
    uriUnescapedString = uriUnescape<string>(uribmEscapedString);
...
TEST(System, errnoStr) {
  ...
  string expected = strerror(ENOENT);
...
void piecesTest() {
  ...
  VectorType<string,std::allocator<string> > blah;
