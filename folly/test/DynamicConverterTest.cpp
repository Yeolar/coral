...
TEST(DynamicConverter, simple_builtins) {
  dynamic d1 = "Haskell";
  auto i1 = convertTo<std::string>(d1);
  ...
  auto i4 = convertTo<std::pair<std::string, std::string>>(d4);
...
TEST(DynamicConverter, nested_containers) {
  dynamic d1 = { { 1 }, { }, { 2, 3 } };
  auto i1 = convertTo<std::vector<std::vector<uint8_t>>>(d1);
  decltype(i1) i1b = { { 1 }, { }, { 2, 3 } };
  EXPECT_EQ(i1, i1b);
  ...
  auto i2 = convertTo<std::map<double, std::vector<std::string>>>(d2);
  decltype(i2) i2b =
    { { 3.14, { "3", ".", "1", "4" } },
      { 2.72, { "2", ".", "7", "2" } } };
  EXPECT_EQ(i2, i2b);
...
struct Token {
  ...
  explicit Token(int kind, const std::string& lexeme)
...
template <> struct DynamicConverter<Token> {
  static Token convert(const dynamic& d) {
    ...
    std::string lex = convertTo<std::string>(d["LEXEME"]);
    return Token(k, lex);
  }
};
}
