...
TEST(Uri, Simple) {
  ...
  {
    // test query parameters
    std::string s("http://localhost?key1=foo=bar&key2=foobar&");
    Uri u(s);
    auto paramsList = u.getQueryParams();
    std::map<std::string, std::string> params;
    for (auto& param : paramsList) {
  ...
  {
    std::string s("this:is/a/valid/uri");
    Uri u(s);
  ...
  {
    std::string s("this:is:another:valid:uri");
    Uri u(s);
    EXPECT_EQ("this", u.scheme());
...
BENCHMARK(init_uri_simple, iters) {
  const std::string s("http://localhost?&key1=foo&key2=&key3&=bar&=bar=&");
